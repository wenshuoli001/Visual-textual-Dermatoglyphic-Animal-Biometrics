...
for j in range(10):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original_bifurcation/b3.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[56,49], [76,30], [63,81], [89,113]])
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/b3' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/b3' + str(j) + '.txt','w') as f:
