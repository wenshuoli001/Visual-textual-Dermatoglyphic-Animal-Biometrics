...
for j in range(5):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original convergence/c3.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[73,48],[65,173],[111,162],[114,289]])
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/c3' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/c3' + str(j) + '.txt','w') as f:
