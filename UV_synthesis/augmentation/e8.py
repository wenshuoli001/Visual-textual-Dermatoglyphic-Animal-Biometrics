...
for j in range(20):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original_enclosures/e8.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[26,6], [17,20], [23,32], [12,92]])
    ...
    for i in range(len(src_points)):
        img = localTranslationWarpFastWithStrength(img, src_points[i][0], src_points[i][1], dst_points[i][0], dst_points[i][1], 100, 300)
    ...
    #定义矩阵变换关键点的新的随机值
    '''此处随机值的大小代表了仿射变换的程度'''
    rand_val = np.random.randint(1, 2)
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/e8' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/e8' + str(j) + '.txt','w') as f:
