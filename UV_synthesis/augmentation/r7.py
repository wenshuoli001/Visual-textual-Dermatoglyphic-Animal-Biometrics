...
for j in range(5):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original_ridges/r7.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[68,23], [74,141]])
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/r7' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/r7' + str(j) + '.txt','w') as f:
