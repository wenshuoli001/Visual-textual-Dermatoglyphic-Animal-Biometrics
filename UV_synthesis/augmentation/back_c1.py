...
for j in range(7):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('back_stripes/back_c/c1.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[47,8], [31,28], [5,123], [29,273]])
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_back/c1' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_back/c1' + str(j) + '.txt','w') as f:
