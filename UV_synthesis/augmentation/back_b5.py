...
for j in range(10):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('back_stripes/back_b/b5.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[39,18],[26,81],[33,106],[15,228]])
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_back/b5' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_back/b5' + str(j) + '.txt','w') as f:
