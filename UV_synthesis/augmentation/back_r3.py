...
for j in range(8):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('back_stripes/back_r/r3.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[32,14], [18,189]])

    # 获取图像中心坐标
    #center = (warped_img.shape[1] // 2, warped_img.shape[0] // 2)
    center = (23,107)

    # 生成随机值
    rand_val = np.random.randint(1, 50)
    ...
    for i in range(len(src_points)):
        img = localTranslationWarpFastWithStrength(img, src_points[i][0], src_points[i][1], dst_points[i][0], dst_points[i][1], 200, 200)
    ...
    # 生成随机旋转角度和缩放因子
    angle = np.random.uniform(10, 20)  # 旋转角度
    scale = np.random.uniform(0.4, 0.8)  # 缩放因子
    scale = 0.5
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_back/r3' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_back/r3' + str(j) + '.txt','w') as f:
