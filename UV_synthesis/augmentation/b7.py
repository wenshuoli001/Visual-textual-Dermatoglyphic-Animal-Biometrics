...
for j in range(7):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original_bifurcation/b7.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[16,21], [21,62], [22,78], [27,93]])
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/b7' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/b7' + str(j) + '.txt','w') as f:
