...
for j in range(5):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original_bifurcation/b5.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[30,26], [40,59], [24,141], [32,162]])
    ...
    # 生成随机值
    '''此处随机值的大小代表了液化的移动程度'''
    rand_val = np.random.randint(1, 15)
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/b5' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/b5' + str(j) + '.txt','w') as f:
