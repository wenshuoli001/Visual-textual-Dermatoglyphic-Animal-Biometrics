...
for j in range(8):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original convergence/c4.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[28,16],[18,94],[43,152],[32,117]])
    ...
    # 生成随机旋转角度和缩放因子
    '''旋转角度和缩放因子'''
    angle = np.random.uniform(10, 15)  # 旋转角度
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/c4' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/c4' + str(j) + '.txt','w') as f:
