...
for j in range(5):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original_enclosures/e1.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[86,58], [75,123], [112,124], [94,182]])
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/e1' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/e1' + str(j) + '.txt','w') as f:
