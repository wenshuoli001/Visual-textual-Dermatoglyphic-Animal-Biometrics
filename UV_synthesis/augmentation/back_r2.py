...
for j in range(8):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('back_stripes/back_r/r2.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[13,12], [7,130]])
    ...
    # 生成随机值
    rand_val = np.random.randint(1, 25)
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_back/r2' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_back/r2' + str(j) + '.txt','w') as f:
