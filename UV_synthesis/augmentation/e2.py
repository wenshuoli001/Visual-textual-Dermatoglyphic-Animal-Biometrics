...
for j in range(5):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original_enclosures/e2.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[127,64], [116,180], [154,180], [135,263]])
    ...
    # 生成随机值
    '''此处随机值的大小代表了液化的移动程度'''
    rand_val = np.random.randint(1, 20)
    ...
    for i in range(len(src_points)):
        img = localTranslationWarpFastWithStrength(img, src_points[i][0], src_points[i][1], dst_points[i][0], dst_points[i][1], 80, 300)
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/e2' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/e2' + str(j) + '.txt','w') as f:
