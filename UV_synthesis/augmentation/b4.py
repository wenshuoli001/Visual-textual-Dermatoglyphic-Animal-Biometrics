...
for j in range(7):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original_bifurcation/b4.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[45,25], [65,31], [89,35], [84,106]])
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/b4' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/b4' + str(j) + '.txt','w') as f:
