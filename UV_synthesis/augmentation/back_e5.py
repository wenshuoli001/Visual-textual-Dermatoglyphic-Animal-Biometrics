...
for j in range(10):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('back_stripes/back_e/e5.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[19,39], [20,98], [26,115], [9,191]])
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_back/e5' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_back/e5' + str(j) + '.txt','w') as f:
