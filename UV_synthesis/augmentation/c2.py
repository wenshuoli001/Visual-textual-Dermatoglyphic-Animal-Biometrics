...
for j in range(5):
    ...
    # 读取图像，包括透明度通道
    img = cv2.imread('original convergence/c2.png', cv2.IMREAD_UNCHANGED)

    # 定义原始关键点
    src_points = np.int_([[127,74],[107,179],[153,263],[117,404]])
    ...
    #保存第j次变换后的图像
    cv2.imwrite('stripes_part1/c2' + str(j) + '.png', copyImg)

    #将y_distance保存到文件中
    with open('stripes_part1/c2' + str(j) + '.txt','w') as f:
